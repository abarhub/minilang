...
fn interp_identity_typed() {
    assert_eq!(
        ok("int main() { fn(int) -> int f = x => x; return f(7); }"),
        7
    );
}
...
fn interp_add_typed() {
    assert_eq!(
        ok("int main() { fn(int,int) -> int add = (a, b) => a + b; return add(3, 4); }"),
        7
    );
}
...
fn interp_no_param() {
    assert_eq!(
        ok("int main() { fn() -> int get42 = () => 42; return get42(); }"),
        42
    );
}
...
fn interp_type_alias() {
    assert_eq!(
        ok("type F = fn(int) -> int; int main() { F f = x => x * 3; return f(14); }"),
        42
    );
}
...
fn interp_wrong_arg_count_fails() {
    assert!(
        run_source("int main() { fn(int,int) -> int f = (a,b) => a+b; return f(1); }").is_err()
    );
}
