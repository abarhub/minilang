...
fn example_errors_missing_interface_method() {
    ...
    assert!(
        all.contains("getValue") || all.contains("BadImpl") || all.contains("Printable"),
...
fn example_errors_type_mismatch_int_to_string() {
    let all = errors_for_example().join("\n");
    assert!(
        all.contains("int") || all.contains("string") || all.contains("incompatible"),
...
fn example_errors_unknown_method() {
    let all = errors_for_example().join("\n");
    assert!(
        all.contains("fly") || all.contains("méthode") || all.contains("method"),
...
fn example_errors_non_bool_condition() {
    let all = errors_for_example().join("\n");
    assert!(
        all.contains("bool") || all.contains("condition") || all.contains("int"),
