...
fn sources_prefix(cfg: &config::ProjectConfig, cfg_path: &Option<PathBuf>) -> String {
    let Some(includes) = &cfg.sources.include else {
        return String::new();
    };
    let base: PathBuf = cfg_path
        .as_ref()
    ...
    for inc in includes {
        ...
        match fs::read_to_string(&p) {
            ...
            Err(e) => {
                eprintln!(
                    "[sources] include — impossible de lire '{}' : {}",
                    p.display(),
                    e
                );
                process::exit(1);
...
fn run_mode(args: &[String]) {
    ...
    let path: PathBuf = match cli_file {
        ...
        None => match (&cfg.project.main, &cfg_path) {
            (Some(main), Some(cp)) => cp.parent().unwrap_or(Path::new(".")).join(main),
            _ => {
                error!(
                    "Usage: {} <fichier.mini>  (ou [project] main dans {})",
                    args[0],
                    config::CONFIG_FILE
                );
                process::exit(1);
    ...
    let source = fs::read_to_string(&path).unwrap_or_else(|e| {
        error!("Impossible de lire '{}' : {}", path.display(), e);
        process::exit(1);
    });
    ...
    let skip = mini_parser::parser::program_parser()
        .parse(prefix.as_str())
...
fn collect_mini_files(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for p in paths {
        if p.is_dir() {
            collect_mini_files(&p, out);
        } else if p.extension().map(|e| e == "mini").unwrap_or(false) {
            out.push(p);
        }
    }
}
...
fn test_mode(target: Option<&str>) -> i32 {
    ...
    match target {
        ...
        Some(t) => {
            eprintln!("'{}' : fichier ou répertoire introuvable", t);
            return 1;
        }
        None => {
            ...
            if !dir.is_dir() {
                eprintln!(
                    "Répertoire de tests introuvable : {} \
    ...
    for file in &files {
        ...
        let source = match fs::read_to_string(file) {
            Ok(s) => s,
            Err(e) => {
                println!("  erreur : lecture impossible : {}", e);
                file_errors += 1;
                continue;
            }
        };
    ...
    if file_errors > 0 {
        bilan.push_str(&format!(", {} fichier(s) en erreur", file_errors));
    }
    println!("{}", bilan);
    if failures > 0 || file_errors > 0 {
        1
    } else {
        0
    }
...
fn print_program(p: &Program, skip: &DeclCounts) {
    ...
    for alias in &aliases {
        println!("type {} = {};", alias.name, alias.ty);
    }
    if !aliases.is_empty() {
        println!();
    }
    for m in p.modules.iter().skip(skip.modules) {
        print_module(m);
        println!();
    }
    for iface in p.interfaces.iter().skip(skip.ifaces) {
        print_interface(iface);
        println!();
    }
    for e in p.enums.iter().skip(skip.enums) {
        print_enum(e);
        println!();
    }
    for c in p.classes.iter().skip(skip.classes) {
        print_class(c);
        println!();
    }
...
fn fmt_stmt_inline(s: &Stmt) -> String {
    match s {
        Stmt::VarDecl {
            ...
            if let Some(e) = init {
                format!("{}{} {} = {}", qualifier, ty, name, fmt_expr(e))
            } else {
...
fn fmt_expr(e: &Expr) -> String {
    match e {
        ...
        Expr::FieldAccess { object, field } => format!("{}.{}", fmt_expr(object), field),
        Expr::MethodCall {
            object,
            method,
            args,
        ...
        Expr::EnumConstructor {
            ...
            let a: Vec<String> = args.iter().map(fmt_expr).collect();
            if a.is_empty() {
                format!("{}{}::{}", enum_name, ta, variant)
            } else {
                format!("{}{}::{}({})", enum_name, ta, variant, a.join(", "))
            }
        }
        ...
        Expr::SafeFieldAccess { object, field } => format!("{}?.{}", fmt_expr(object), field),
        Expr::SafeMethodCall {
            object,
            method,
            args,
