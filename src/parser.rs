...
fn type_parser() -> impl Parser<char, Type, Error = Simple<char>> + Clone {
    recursive(|ty| {
        ...
        .map(|((t, v), opt)| {
            ...
            if opt.is_some() {
                Type::Generic("Option".to_string(), vec![t])
            } else {
                t
            }
        })
...
fn record_def_parser() -> chumsky::BoxedParser<'static, char, RecordDef, Simple<char>> {
    ...
    let braced_block: chumsky::BoxedParser<char, Vec<Stmt>, Simple<char>> = just('{')
        .padded_by(ws())
        .ignore_then(take_until(just('}')))
        .map(|_| vec![Stmt::Builtin])
        .or(text::keyword("builtin")
            .padded_by(ws())
            .then_ignore(just(';').padded_by(ws()))
            .map(|_| vec![Stmt::Builtin]))
        .boxed();
    ...
    .or_not()
    .map(|q| q.unwrap_or(Qualifier::Mutable))
    .then(text::ident().padded_by(ws()))
    .separated_by(just(',').padded_by(ws()))
    .at_least(1)
    .delimited_by(just('<').padded_by(ws()), just('>').padded_by(ws()))
    ...
    .or_not()
    .map(|v| v.unwrap_or(Visibility::Public))
    ...
    let record_method = vis_mut
        ...
        .then(braced_block)
        .map(|(((((visibility, is_mutable), rt), n), p), b)| Method {
            visibility,
            is_mutable,
            return_type: rt,
            name: n,
    ...
    kw("record")
        ...
        .map(|((((name, tp), fields), implements), methods)| {
            let type_param_constraints: Vec<(String, Qualifier)> = tp
                .iter()
...
pub fn program_parser() -> impl Parser<char, Program, Error = Simple<char>> {
    ...
    let expr: BoxedParser<char, Expr, Simple<char>> = recursive(|expr| {
        ...
        let str_lit = ml_str_lit.or(just('"')
        ...
        let number_lit = text::int(10)
            ...
            .map(
                |(i, maybe_frac): (String, Option<String>)| match maybe_frac {
        ...
        let paren_or_call = expr
            .clone()
            .delimited_by(just('(').padded_by(ws()), just(')').padded_by(ws()))
            .then(call_args.clone().or_not())
            .map(|(e, maybe_args)| match maybe_args {
                None => e,
                Some(a) => Expr::LambdaCall {
                    callee: Box::new(e),
                    args: a,
                },
            });
        ...
        let array_lit = kw("new")
            ...
            .try_map(|(t, elems), span| match t {
                Type::Array(inner) => Ok(Expr::ArrayLit {
                    elem_type: *inner,
                    elements: elems,
                }),
        ...
        enum Postfix {
            ...
            SafeField(String),
            SafeMethod(String, Vec<Expr>),
            Index(Box<Expr>),
        }

        let safe_postfix_op = just("?.")
            .padded_by(ws())
            .ignore_then(text::ident().padded_by(ws()))
            .then(call_args.clone().or_not())
            .map(|(name, args)| match args {
        ...
        let postfix = atom
            ...
            .foldl(|obj, pf| match pf {
                Postfix::Field(f) => Expr::FieldAccess {
                    object: Box::new(obj),
                    field: f,
                },
                Postfix::Method(m, a) => Expr::MethodCall {
                    object: Box::new(obj),
                    method: m,
                    args: a,
                },
                Postfix::SafeField(f) => Expr::SafeFieldAccess {
                    object: Box::new(obj),
                    field: f,
                },
                Postfix::SafeMethod(m, a) => Expr::SafeMethodCall {
                    object: Box::new(obj),
                    method: m,
                    args: a,
                },
                Postfix::Index(i) => Expr::Index {
                    object: Box::new(obj),
                    index: i,
                },
            });
        ...
        let lambda_block = stmt_fwd
            .clone()
            .repeated()
            .delimited_by(just('{').padded_by(ws()), just('}').padded_by(ws()));

        // Corps = bloc  ou  expr récursif (supporte x => y => z)
        let lambda_body_p = lambda_block
            .map(LambdaBody::Block)
            .or(expr.clone().map(|e| LambdaBody::Expr(Box::new(e))));

        let lambda_multi = text::ident()
            .padded_by(ws())
            .separated_by(just(',').padded_by(ws()))
            .allow_trailing()
            .delimited_by(just('(').padded_by(ws()), just(')').padded_by(ws()))
        ...
        let null_coal = arith
            ...
            .then(
                just("??")
                    .padded_by(ws())
                    .ignore_then(arith.clone())
                    .or_not(),
            )
            .map(|(e, d)| match d {
                None => e,
                Some(def) => Expr::NullCoalesce {
                    expr: Box::new(e),
                    default: Box::new(def),
                },
            });
    ...
    let stmt_impl: BoxedParser<char, Stmt, Simple<char>> = recursive(|stmt| {
        ...
        let kw_var_decl = qualifier
            .clone()
            .then(kw_type.clone())
            .then(text::ident().padded_by(ws()))
            .then(just('=').padded_by(ws()).ignore_then(expr.clone()).or_not())
            .then_ignore(just(';').padded_by(ws()))
            .map(|(((qualifier, ty), name), init)| Stmt::VarDecl {
                qualifier,
                ty,
                name,
                init,
        ...
        let generic_var_decl = qualifier
            .clone()
            .then(type_parser())
            .then(text::ident().padded_by(ws()))
            .then(just('=').padded_by(ws()).ignore_then(expr.clone()).or_not())
    ...
    let method_body = body.clone().or(kw("builtin")
        .then_ignore(just(';').padded_by(ws()))
        .to(vec![Stmt::Builtin]));
    ...
    let enum_type_params = type_param_entry
        .clone()
        .separated_by(just(',').padded_by(ws()))
        .at_least(1)
        .delimited_by(just('<').padded_by(ws()), just('>').padded_by(ws()))
    ...
    let method_sig = kw("mutable")
        ...
        .then_ignore(just(';').padded_by(ws()))
        .map(|(((is_mutable, rt), n), p)| MethodSig {
            is_mutable,
            return_type: rt,
            name: n,
            params: p,
    ...
    let type_param_list = type_param_entry
        .clone()
        .separated_by(just(',').padded_by(ws()))
        .at_least(1)
        .delimited_by(just('<').padded_by(ws()), just('>').padded_by(ws()))
    ...
    let class_def = kw("transient")
        ...
        .map(
            |(((((((is_transient, is_service), is_mut), name), tp), parent), impls), members)| {
                let mut fields = vec![];
                let mut ctors = vec![];
                let mut methods = vec![];
                for m in members {
    ...
    let import_path = text::ident()
        .then(
            just('.')
                .ignore_then(text::ident().or(just('*').to("*".to_string())))
                .repeated()
                .at_least(1),
        )
        .map(|(h, t)| {
            let mut p = vec![h];
            p.extend(t.iter().cloned());
    ...
    let func_def = kw("test")
        ...
        .then(body.clone())
        .map(|((((is_test, rt), name), p), b)| FuncDef {
            is_test,
            return_type: rt,
            name,
            params: p,
    ...
    ws().ignore_then(top_decl.repeated())
